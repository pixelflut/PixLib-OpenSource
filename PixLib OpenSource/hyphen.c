...
static char * get_state_str(int state, int level) {
  int i;
  HashEntry *e;
    ...
    for (j = 0; i < lhmin && word[j] != '\0'; i++) do {
      ...
      if (*rep && *pos && *cut && (*rep)[j]) {
        ...
        if (rh && (hnj_hyphen_strnlen(word, j - (*pos)[j] + 1, utf8) +
          hnj_hyphen_strnlen((*rep)[j], (int)rh - (int)(*rep)[j], utf8)) < lhmin) {
            free((*rep)[j]);
            (*rep)[j] = NULL;
            hyphens[j] = '0';
          }
    ...
    for (j = word_size - 1; i < rhmin && j > 0; j--) {
      ...
      if (*rep && *pos && *cut && (*rep)[j]) {
        ...
        if (rh && (hnj_hyphen_strnlen(word + j - (*pos)[j] + (*cut)[j] + 1, 100, utf8) +
          hnj_hyphen_strnlen(rh + 1, (int)strlen(rh + 1), utf8)) < rhmin) {
            free((*rep)[j]);
            (*rep)[j] = NULL;
            hyphens[j] = '0';
          }
...
int hnj_hyphen_hyph_(HyphenDict *dict, const char *word, int word_size,
  ...
  int prep_word_size = word_size + 3;
